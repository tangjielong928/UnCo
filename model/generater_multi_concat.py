...
class SequenceGeneratorModel(nn.Module):
    ...
    def forward(self, src_tokens,image_feature, tgt_tokens, src_seq_len=None, tgt_seq_len=None, first=None, candidate_regions=None, gt_entities=None):
        """
        ...
        return self.seq2seq_model(src_tokens, image_feature,tgt_tokens, src_seq_len, tgt_seq_len, first, candidate_regions, gt_entities)
    ...
    def predict_with_logits(self, src_tokens, image_feature, src_seq_len=None, first=None):
        """
        给定source的内容，输出generate过程中的logits，用于计算uncertainty
        
        :param torch.LongTensor src_tokens: bsz x max_len
        :param torch.LongTensor src_seq_len: bsz
        :return: dict包含logits和region_pred
        """
        img_feat_, state = self.seq2seq_model.prepare_state(src_tokens, image_feature, src_seq_len, first)
        result, region_result, pred_logits, region_logits = self.generator.generate_with_logits(img_feat_, state)
        return {'pred': result, 'region_pred': region_result, 'pred_logits': pred_logits, 'region_logits': region_logits}
...
class SequenceGenerator:
    ...
    @torch.no_grad()
    def generate_with_logits(self, img_feat_, state, tokens=None):
        """
        与generate相同，但同时返回生成过程中的logits，用于计算uncertainty
        
        :param State state: encoder结果的State, 是与Decoder配套是用的
        :param torch.LongTensor,None tokens: batch_size x length, 开始的token
        :return: bsz x max_length' 生成的token序列及其logits
        """
        # 创建一个新的generate函数，返回logits
        generate_with_logits_func = partial(greedy_generate_with_logits, decoder=self.decoder, max_length=self.max_length, 
                                          max_len_a=self.max_len_a, num_beams=self.num_beams,
                                          bos_token_id=self.bos_token_id, eos_token_id=self.eos_token_id,
                                          repetition_penalty=self.repetition_penalty,
                                          length_penalty=self.length_penalty, pad_token_id=self.pad_token_id,
                                          restricter=self.restricter, top_k=1)
        
        return generate_with_logits_func(img_feat_=img_feat_, tokens=tokens, state=state)
...
@torch.no_grad()
def greedy_generate_with_logits(decoder, tokens=None, state=None, img_feat_=None, max_length=20, max_len_a=0.0, num_beams=1,
                              bos_token_id=None, eos_token_id=None, pad_token_id=0,
                              repetition_penalty=1, length_penalty=1.0, restricter=None, top_k=1):
    """
    与greedy_generate类似，但同时返回生成过程中的logits
    """
    if num_beams == 1:
        token_ids, region_ids, all_logits, region_logits = _no_beam_search_generate_with_logits(
            decoder, img_feat_=img_feat_, tokens=tokens, state=state, max_length=max_length, max_len_a=max_len_a,
            bos_token_id=bos_token_id, eos_token_id=eos_token_id,
            repetition_penalty=repetition_penalty, length_penalty=length_penalty,
            pad_token_id=pad_token_id, restricter=restricter, top_k=top_k
        )
    else:
        # 如果使用beam search，当前暂不支持返回logits
        token_ids = _beam_search_generate(decoder, tokens=tokens, state=state, max_length=max_length, max_len_a=max_len_a,
                                         num_beams=num_beams,
                                         bos_token_id=bos_token_id, eos_token_id=eos_token_id, do_sample=False,
                                         repetition_penalty=repetition_penalty, length_penalty=length_penalty,
    ...
    return token_ids, region_ids, all_logits, region_logits
...
def _no_beam_search_generate_with_logits(decoder, state, img_feat_=None, tokens=None, max_length=20, max_len_a=0.0, bos_token_id=None,
                                       eos_token_id=None, repetition_penalty=1.0, length_penalty=1.0, pad_token_id=0,
                                       restricter=None, top_k=1):
    """
    与_no_beam_search_generate类似，但同时返回生成过程中的logits
    """
    device = _get_model_device(decoder)
    region_threshold = 0.0
    ...
    all_logits = []
    region_logits = []
    scores, region_scores = decoder.decode(img_feat_=img_feat_, tokens=tokens, state=state)
    all_logits.append(scores)  # 保存当前步骤的logits
    region_logits.append(region_scores)  # 保存当前步骤的region logits
    ...
    while cur_len < real_max_length:
        ...
        # 将所有已经生成完成的sample的生成结果设置为特殊的结束符号
        if eos_token_id is not None:
            scores = scores.masked_fill(dones.unsqueeze(1).expand_as(scores), -1e32) 
        ...
        if restricter is not None:
            _, next_tokens = restricter(state, token_ids, scores, num_beams=1)
        else:
            next_tokens = scores.argmax(dim=-1, keepdim=True)
        ...
        next_tokens = next_tokens.masked_fill(dones.unsqueeze(1), pad_token_id)
        tokens = next_tokens
        token_ids = torch.cat([token_ids, tokens], dim=1)
        
     
        dones = dones.__or__(tokens.squeeze(1).eq(eos_token_id))
        ...
        if dones.min() == 1:
            break
        if cur_len == max_lengths.max().item():
            break
        
    # 将logits转换为tensor并按生成顺序拼接
    all_logits = torch.stack(all_logits, dim=1)  # [batch_size, seq_len, vocab_size]
    region_logits = torch.stack(region_logits, dim=1)  # [batch_size, seq_len, region_size]
    
    # 如果存在长度不够的sample，在token_ids后面pad token
    for idx, (length, max_length) in enumerate(zip(torch.sum(token_ids.ne(pad_token_id), dim=1), max_lengths)):
        if length < max_length:
            token_ids[idx, length:] = pad_token_id
