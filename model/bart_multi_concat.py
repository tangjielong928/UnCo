...
def compute_ious(gt_box, region_boxes):
    # gt_box: [4], region_boxes: [num_regions, 4]
    xA = torch.max(gt_box[0], region_boxes[:, 0])
    yA = torch.max(gt_box[1], region_boxes[:, 1])
    xB = torch.min(gt_box[2], region_boxes[:, 2])
    yB = torch.min(gt_box[3], region_boxes[:, 3])
    interArea = (xB - xA).clamp(min=0) * (yB - yA).clamp(min=0)
    boxAArea = (gt_box[2] - gt_box[0]) * (gt_box[3] - gt_box[1])
    boxBArea = (region_boxes[:, 2] - region_boxes[:, 0]) * (region_boxes[:, 3] - region_boxes[:, 1])
    iou = interArea / (boxAArea + boxBArea - interArea + 1e-6)
    return iou

def compute_triplet_loss(entity_feat, region_feats, ious, lama=0.5):
    pos_mask = ious > 0.5
    neg_mask = ious < 0.5
    if pos_mask.sum() == 0 or neg_mask.sum() == 0:
        return 0.0
    pos_feats = region_feats[pos_mask]
    neg_feats = region_feats[neg_mask]
    pos_ious = ious[pos_mask]
    loss = 0.0
    count = 0
    for i in range(pos_feats.size(0)):
        for j in range(neg_feats.size(0)):
            pos_dist = F.pairwise_distance(entity_feat.unsqueeze(0), pos_feats[i].unsqueeze(0))
            neg_dist = F.pairwise_distance(entity_feat.unsqueeze(0), neg_feats[j].unsqueeze(0))
            margin = lama*(1-pos_ious[i])
            loss = loss + F.relu(margin + pos_dist - neg_dist)
            count += 1
    if count > 0:
        loss = loss / count
    return loss
...
class FBartEncoder(Seq2SeqEncoder):
    ...
    def forward(self, src_tokens, image_feature, src_seq_len, candidate_regions=None, gt_entities=None):
        ...
        triplet_loss = 0.0
        if candidate_regions is not None and gt_entities is not None:
            box_num = candidate_regions.size(1)
            for b in range(src_tokens.size(0)):
                region_feats = encoder_outputs[b, :box_num, :]
                region_boxes = candidate_regions[b]  # [box_num, 4]
                for ent in gt_entities[b]:
                    start, end = ent['start'], ent['end']
                    if end < start: continue
                    entity_feat = encoder_outputs[b, start:end+1, :].mean(dim=0)
                    gt_box = torch.tensor(ent['gt_box'], dtype=region_boxes.dtype, device=region_boxes.device)
                    ious = compute_ious(gt_box, region_boxes)
                    triplet_loss = triplet_loss + compute_triplet_loss(entity_feat, region_feats, ious)
            triplet_loss = triplet_loss / src_tokens.size(0)
        return img_feat_, encoder_outputs, multi_modal_mask, hidden_states, triplet_loss
...
class BartSeq2SeqModel(Seq2SeqModel):
    ...
    def prepare_state(self, src_tokens, image_feature,src_seq_len=None, first=None, tgt_seq_len=None):
        img_feat_, encoder_outputs, encoder_mask, hidden_states, triplet_loss = self.encoder(src_tokens,image_feature, src_seq_len)
        src_embed_outputs = hidden_states[0]
        state = BartState(encoder_outputs, encoder_mask, src_tokens, first, src_embed_outputs, triplet_loss)
...
class BartState(State):
    def __init__(self, encoder_output, encoder_mask, src_tokens, first, src_embed_outputs, triplet_loss):
